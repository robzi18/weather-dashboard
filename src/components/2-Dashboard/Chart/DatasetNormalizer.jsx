...
const DatasetNormalizer = ({ forecast, isPerc, isTemp, isCel }) => {
  ...
  const [chartPrecipData, setChartPrecipData] = useState({
    ...
    datasets: [
      {
        label: "Rainfall",
        data: nextHours?.map((nextH) => Math.round(nextH?.precip_mm)), //I wanted to show dynamically but not working
        fill: true,
  ...
  const [chartTempDataC, setChartTempDataC] = useState({
    ...
    datasets: [
      {
        label: "Temperature (°C)",
        data: nextHours?.map((nextH) => Math.round(nextH?.temp_c)), //I wanted to show dynamically but not working
        fill: true,
  ...
  const [chartTempDataF, setChartTempDataF] = useState({
    ...
    datasets: [
      {
        label: "Temperature (°F)",
        data: nextHours?.map((nextH) => Math.round(nextH?.temp_f)), //I wanted to show dynamically but not working
        fill: true,
